...
public class PomTransformerTest {
    ...
    void modifySurefirePlugin_shouldAddSurefirePlugin() throws IOException, XmlPullParserException {
        ...
        // assert
        assertThatSurefirePluginIsPresent(transformer.getModel().getBuild().getPlugins().get(0)) ;
    }
    ...
    void modifySurefirePlugin_shouldModifyArgLine() throws XmlPullParserException, IOException {
        ...
        PomTransformer transformer = new PomTransformer(testPom);
        Xpp3Dom surefireConfiguration = (Xpp3Dom) transformer.getModel().getBuild().getPlugins().get(0).getConfiguration();
    ...
    @Test
    void modifySurefirePlugin_shouldAdd_failIfNoTests_and_testFailureIgnore() throws XmlPullParserException, IOException {
        // arrange
        Path testPom = Path.of("src/test/resources/surefire/add-failIfNoTests_testFailureIgnore.xml");
        PomTransformer transformer = new PomTransformer(testPom);
        Xpp3Dom originalSurefireConfiguration = (Xpp3Dom) transformer.getModel().getBuild().getPlugins().get(0).getConfiguration();
        ...
        // assert
        Model transformedModel = transformer.getModel();
        Xpp3Dom transformedSurefireConfiguration = (Xpp3Dom) transformedModel.getBuild().getPlugins().get(0).getConfiguration();
        assertThat(transformedSurefireConfiguration.getChildCount()).isEqualTo(3);

        String failIfNoTests = transformedSurefireConfiguration.getChild("failIfNoTests").getValue();
        assertThat(failIfNoTests).asBoolean().isFalse();
        String testFailureIgnore = transformedSurefireConfiguration.getChild("testFailureIgnore").getValue();
        assertThat(testFailureIgnore).asBoolean().isTrue();
    }

    @Test
    void modifySurefirePlugin_shouldModify_failIfNoTests_and_testFailureIgnore() throws XmlPullParserException, IOException {
        // arrange
        Path testPom = Path.of("src/test/resources/surefire/modify-failIfNoTests_testFailureIgnore.xml");
        PomTransformer transformer = new PomTransformer(testPom);
        Xpp3Dom surefireConfiguration = (Xpp3Dom) transformer.getModel().getBuild().getPlugins().get(0).getConfiguration();
        String originalFailIfNoTests = surefireConfiguration.getChild("failIfNoTests").getValue();
        assertThat(originalFailIfNoTests).asBoolean().isTrue();
        String originalTestFailureIgnore = surefireConfiguration.getChild("testFailureIgnore").getValue();
        ...
        String transformedFailIfNoTests = surefireConfiguration.getChild("failIfNoTests").getValue();
        assertThat(transformedFailIfNoTests).asBoolean().isFalse();
        String transformedTestFailureIgnore = surefireConfiguration.getChild("testFailureIgnore").getValue();
